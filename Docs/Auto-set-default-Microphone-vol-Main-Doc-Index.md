...
| [Features/Device-Priority-Profiles-Hotkeys](Features/Device-Priority-Profiles-Hotkeys.md) | Shipped v1.5 feature doc: capture+render priority lists, profiles, fallback alerts, volume hotkeys + OSD; v1.6 adds the mixer popup, boost-past-100%, and the active-window hotkey target; v1.7 adds nav modes (digits/arrows/WASD), the all-sessions rolodex, live level pulse, and M mute; v1.9 adds profile-switch hotkeys (`profile:next` / `profile:<name>`, shared `App.set_profile` path, OSD text-note mode) — API surface, config, testing, troubleshooting |
...
| [superpowers/plans/2026-07-17 event-history](superpowers/plans/2026-07-17-event-history.md) | v1.9 implementation plan — 4 TDD tasks (`history_push`/`HistoryRecorder` pure core + persistence, 9 call sites, Settings History card + Clear, docs/backlog) |
