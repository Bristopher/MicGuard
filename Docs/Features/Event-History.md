...
| `fallback` | `App.notify_fallback` (called from `Enforcer._enforce_flow`'s `on_fallback`) | availability-driven device switch where the new device is NOT the flow's #1 priority entry, or nothing connected at all — suppressed (final-review fix, 2026-07-17) when the switch is caused by a deliberate profile change (`Enforcer._last_profile`/`profile_changed`), so a hotkey/tray/settings profile switch does NOT also produce a `fallback` row |
